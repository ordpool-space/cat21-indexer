...
- cat21 has no inscribe surface. If one is added, bind its fee to the orchestrator's `simulations[].preview` (commit/reveal split), NOT a `CandidateFeeRow`; inscribe reports `absorbedSubDustSats: null` even when it CAN fund, so ask the SDK to surface the fold rather than deriving it.
