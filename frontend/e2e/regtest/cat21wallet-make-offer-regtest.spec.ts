...
/**
 * The asset-notice state on make-offer: a DIRTY-ONLY buyer funding pool on a
 * SEPARATE-address wallet (cat21wallet) NOTICEs and PROCEEDS, per the asset-to-
 * miner ruling. The old expert-required path left the CTA disabled behind a block
 * warning for this exact wallet; the proof is that make-offer-asset-notice renders,
 * names the asset, and make-offer-cta is ENABLED. Requires the 9e578ef pin (the
 * create-offer orchestrator now carries the asset-notice simulation). The shared
 * picker's three fee states are captured on the transfer notice cell (same
 * component); this cell is the surface-specific notice + enabled-CTA gate.
 */
async function runMakeOfferNoticeCell(): Promise<void> {
  const tag = 'make-offer:notice';
  const PRICE_SATS = 1_000;
  const CAT_VALUE = 546;
  const DIRTY_SATS = 6_000; // covers price + fee at 5 sat/vB; dirty-ONLY (no clean coin)

  const page = await context.newPage();
  const { paymentAddress: buyerPayment } = await connectAndReadAddresses(page);
  console.log(`[${tag}] buyerPayment=${buyerPayment}`);

  // DIRTY-ONLY buyer funding: the only covering coin carries an asset, so no clean
  // coin covers price + cat value + fee — on a separate-address wallet that is
  // asset-notice (notice-and-proceed), NOT the one-address block.
  ...
  // §7.8 dirty-only premise: cat21wallet's fixed-seed payment address accumulates
  // across runs; a prior run's clean covering coin would take the `auto` branch
  // and no notice renders. Log the pre-notice scan; the failure message names it.
  ...
  await expect(
    notice,
    `no make-offer-asset-notice. Suspect, in order: (1) a clean covering coin accumulated on cat21wallet's fixed-seed payment address from a prior LOCAL run — reset the regtest chain (§7.8 shared-seed trap); (2) topology mis-derived — resolveFundingTopology('derive', wallet) needs both paymentAddress + ordinalsAddress; (3) the seeded coin did not classify dirty`,
